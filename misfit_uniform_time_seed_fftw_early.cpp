//compile: mpicc -I /home_soft/home/hbjkong/fftw3/include -o test ffc.cpp -lfftw3_mpi -lfftw3 -lm -L /home_soft/home/hbjkong/fftw3/lib
//Date: 2014.09.25
...
static void PrintUsage(const char* argv0) {
    cerr
        ...
        << "  --steps <int>       Total iterations (default: 5002)\n"
        << "  --mod <int>         Output/analysis interval (default: 25)\n"
        << "  --grainx <int>      Initial grains in x (default: 80)\n"
        << "  --grainy <int>      Initial grains in y (default: 80)\n"
...
static int grainx=80,grainy=80,grainz=1;
				...
				for (int l = 0; l < dlocal_n0; l++) {
					for (int m = 0; m < 160; m++) {
						for (int n = 0; n < N; n++) {
							...
							if (abs(xx) < (L * dx / grainx / 2. - 0.2) && abs(yy) < (M * dx / grainy / 2. - 0.2) && abs(zz) < 4.3) dphi[n + N * (m + M * l)][0] = 2 * A1 * (cos(((1-con0)*ka0+con0*kb0) * X * dx) + cos(((1-con0)*ka0+con0*kb0) * Y * dx)) + 4 * A2 * cos(((1-con0)*ka0+con0*kb0) * X * dx) * cos(((1-con0)*ka0+con0*kb0) * Y * dx) + u0;
						}
					}
				}
			}
