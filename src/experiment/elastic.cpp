...
// Adapted from ref/elastic.cpp for unified CLI framework
...
static float a_lattice=1.0,b_lattice=0.85; //lattice A and B for square

static float u0=0.05,sig=0.05,con0=0.2,dt=0.1,dx=0.125;
...
static float kb0=2.0*pi/b_lattice,kb1=kb0,kb2=kb1*2/sqrt(3.0),sigMb1=0.55,sigMb2=0.55; //for square B
...
static int grainx=2,grainy=2,grainz=1,dnx=L/grainx,dny=M/grainy,dnz=N/grainz;
...
static float bench_l = 0.8, bench_h = 0.94, bench100_l = 0.796, bench100_h = 1.2,benchel=1.7;
...
static void INITIAL_dis(fftw_complex *dphi,fftw_complex *dcon,int dlocal_0_start, int dlocal_n0, int dmyid, int dnumprocs,int dtheta);
...
static void interENERGYC2(fftw_plan pe, fftw_complex *dphi, fftw_complex *dcon, fftw_complex *dconHat, fftw_complex *din3, fftw_complex *din, fftw_complex *dgrac, fftw_complex *dout, double *denei, double &dEEsum, double &dDSsum, int dlocal_n0, int dlocal_0_start, int dmyid, int dnumprocs);
static void halfENERGYC2(fftw_plan pe, fftw_complex *dphi, fftw_complex *dcon, fftw_complex *dconHat, fftw_complex *din3, fftw_complex *din, fftw_complex *dgrac, fftw_complex *dout, double *denei, double &dEEsum, double &dDSsum, int dlocal_n0, int dlocal_0_start, int dmyid, int dnumprocs,int dtheta);
...
static void phimax_atmposi(fftw_complex *dphi,int dlocal_n0,int dlocal_0_start, int dmyid,int dnumprocs);
static void elastic(double *drbuf_x, double *drbuf_y, double *drbuf_z, int dsize_buf); 
static void elastic3D(double *drbuf_x, double *drbuf_y, double *drbuf_z, int dsize_buf);
static void cryst_order(double *drbuf_x, double *drbuf_y, double *drbuf_z, int dsize_buf);
...
static void PrintUsage(const char* argv0) {
    cerr
        << "Usage: " << (argv0 ? argv0 : "pfc-exp-cli") << " --model elastic [options]\n"
        << "Options:\n"
        ...
        << "  --steps <int>       Total iterations (default: 2002)\n"
        << "  --mod <int>         Output/analysis interval (default: 200)\n"
        << "  --seed <uint>       Random seed base (default: 20200604)\n"
        << "  --grainx <int>      Initial grains in x (default: 2)\n"
        << "  --grainy <int>      Initial grains in y (default: 2)\n"
        << "  --grainz <int>      Initial grains in z (default: 1)\n"
        << "  --benchel <float>   Neighbor cutoff for elastic analysis (default: 1.7)\n"
        << "  --input-dir <path>  Directory with pre-computed phi/con data\n"
...
return 0;
        ...
        if (arg == "--grainx" || PFC_HasPrefix(arg, "--grainx=")) {
            const char* v = (arg == "--grainx") ? require_value("--grainx") : (raw + 9);
            if (!PFC_ParseIntArg(v, grainx)) goto bad;
            continue;
        }
        if (arg == "--grainy" || PFC_HasPrefix(arg, "--grainy=")) {
            const char* v = (arg == "--grainy") ? require_value("--grainy") : (raw + 9);
            if (!PFC_ParseIntArg(v, grainy)) goto bad;
            continue;
        }
        if (arg == "--grainz" || PFC_HasPrefix(arg, "--grainz=")) {
            const char* v = (arg == "--grainz") ? require_value("--grainz") : (raw + 9);
            if (!PFC_ParseIntArg(v, grainz)) goto bad;
            continue;
        }
        if (arg == "--benchel" || PFC_HasPrefix(arg, "--benchel=")) {
            const char* v = (arg == "--benchel") ? require_value("--benchel") : (raw + 10);
            if (!PFC_ParseFloatArg(v, benchel)) goto bad;
            continue;
        }
        if (arg == "--input-dir" || PFC_HasPrefix(arg, "--input-dir=")) {
            const char* v = (arg == "--input-dir") ? require_value("--input-dir") : (raw + 12);
            if (!v || *v == '\0') goto bad;
            input_dir = v;
            continue;
        }
...
MPI_Comm_size(MPI_COMM_WORLD, & numprocs);
    ...
    if (myid == 0) {
        ...
        if (run_cfg.is_open()) {
            ...
            run_cfg << "benchel " << benchel << "\n";
            if (!input_dir.empty()) run_cfg << "input_dir " << input_dir << "\n";
            run_cfg.close();
        }
...
static void INITIAL_dis(fftw_complex *dphi,fftw_complex *dcon,int dlocal_0_start, int dlocal_n0, int dmyid, int dnumprocs,int dtheta){
	int x, y, z, lx, X, Y, Z; float xx, yy, zz;
	float A1 = 0.1427, A2 = 0.106;
	float r1, cut1, cut2, qb, theta, k00, r2, k0, w3, theta0;
	...
	int x2 = 380, y2 = 380, z2 = 380;
	w3 = 0.*pi / 180;
	/*if (dtheta == 0) {
		dtheta = 1;
	}	*/
	//theta0 = pi / 2 - dtheta*pi / 900;
	zl = L / 4;// * tan(theta0) - M / 2;
	rl = L * 3 / 4;// * tan(theta0) - M / 2;
	thetad = 36;//����н�
			   //cout << zl << " " << rl << theta0 << endl;
	theta0 = 5;//ƫת�Ƕ�
	   ...
	   for (int l = 0; l < dlocal_n0; l++) {
		   for (int m = 0; m < M; m++) {
			   for (int n = 0; n < N; n++) {
				   ...
				   if (ol<zl - 3 || ol>rl + 3) {
					   theta = thetad * pi / 360;
					   X = (lx - ef)*cos(theta) + m*cos(pi / 2 - theta) + ef;
					   Y = m*cos(theta) - (lx - ef)*cos(pi / 2 - theta) + ef;

					 //  dphi[n + N*(m + M*l)][0] = 2 * A1*(cos(ka0*X*dx) + cos(ka0*Y*dx)) + 4 * A2*cos(ka0*X*dx)*cos(ka0*Y*dx) + u0;
					//   dphi[n + N*(m + M*l)][0] = 2 * A1*(cos(ka0*x*cos(w3) + ka0*y*sin(w3)) + cos(-x*ka0*sin(w3) + ka0*y*cos(w3))) + 4 * A2*cos(ka0*x*cos(w3) + ka0*y*sin(w3))*cos(-x*ka0*sin(w3) + y*ka0*cos(w3)) + u0;
					   dphi[n + N*(m + M*l)][0] = 2 * A1*(cos(ka0*X*dx) + cos(ka0*Y*dx) + cos(ka0*z*dx)) + 4 * A2*(cos(ka0*X*dx)*cos(ka0*Y*dx) + cos(ka0*X*dx)*cos(ka0*z*dx) + cos(ka0*z*dx)*cos(ka0*Y*dx)) + u0;

				   }
				   else if (ol > zl + 3 && ol < rl - 3) {
					   theta = -thetad * pi / 360;
					   X = (lx - ef)*cos(theta) + m*cos(pi / 2 - theta) + ef;
					   Y = m*cos(theta) - (lx - ef)*cos(pi / 2 - theta) + ef;
	...
	for (int l=0; l <dlocal_n0; l ++){
		for (int m=0; m <M; m ++){
		   for (int n=0; n <N; n ++){
			 ...
			 //dC2AHat[n+N*(m+M*l)]=exp(-pow(sig/sigMa1,2))*exp(-pow( sqrt(ks(lnew,m,n,krefl,krefm,krefn))-ka1,2)/(2*gaa1*gaa1))+exp(-pow(sig/sigMa2,2))*exp(-pow( sqrt(ks(lnew,m,n,krefl,krefm,krefn))-ka2,2)/(2*gaa2*gaa2) );
...
static void C2BK(float *dC2BHat, int dlocal_n0, int dlocal_0_start){
	...
	//dC2BHat[n+N*(m+M*l)]=exp(-pow(sig/sigMb1,2))*exp(-pow( sqrt(ks(lnew,m,n,krefl,krefm,krefn))-kb1,2)/(2*gab1*gab1) )+exp(-pow(sig/sigMb2,2))*exp(-pow( sqrt(ks(lnew,m,n,krefl,krefm,krefn))-kb2,2)/(2*gab2*gab2) );
...
static void halfENERGYC2(fftw_plan pe, fftw_complex *dphi, fftw_complex *dcon, fftw_complex *dconHat, fftw_complex *din3, fftw_complex *din, fftw_complex *dgrac, fftw_complex *dout, double *denei, double &dEEsum, double &dDSsum, int dlocal_n0, int dlocal_0_start, int dmyid, int dnumprocs,int dtheta) {
	...
	if (dmyid == 0) {
		...
		energySum = sum_enei / L / M / N;
		char filename[20];
		sprintf(filename, "%s%d%s","half", dtheta, ".txt");
		ofstream wafile(filename, ios_base::out | ios_base::app);
		wafile << kd << " " << sum_enei / L / M / N << " " << sum_dens / L / M / N << endl;
	}
...
static void interENERGYC2(fftw_plan pe, fftw_complex *dphi, fftw_complex *dcon, fftw_complex *dconHat, fftw_complex *din3, fftw_complex *din, fftw_complex *dgrac, fftw_complex *dout, double *denei, double &dEEsum, double &dDSsum, int dlocal_n0, int dlocal_0_start, int dmyid, int dnumprocs) {
	...
	for (int l = 0; l <dlocal_n0; l++) {
		for (int m = 0; m <M; m++) {
			for (int n = 0; n <N; n++) {
				...
				if (x > 10 && x < 110 || x>1500 && x < 1700 || x>3090 && x < 3190) {
					energy_id = pow(dphi[n + N*(m + M*l)][0], 2) / 2.0 - niu*pow(dphi[n + N*(m + M*l)][0], 3) / 6.0 + ksi*pow(dphi[n + N*(m + M*l)][0], 4) / 12.0;
					energy_mix = (dphi[n + N*(m + M*l)][0] + 1.0)*para_w*(dcon[n + N*(m + M*l)][0] * log(dcon[n + N*(m + M*l)][0] / para_c0) + (1.0 - dcon[n + N*(m + M*l)][0])*log((1.0 - dcon[n + N*(m + M*l)][0]) / (1.0 - para_c0)));
					energy_ex = 0.5*dphi[n + N*(m + M*l)][0] * (din3[n + N*(m + M*l)][0]) + para_alpha*dgrac[n + N*(m + M*l)][0];

					denei[n + N*(m + M*l)] = energy_id + energy_mix + energy_ex;//����CPUÿһ���������        
					dEEsum1 += denei[n + N*(m + M*l)];//*dx*dx*dx;                   
					dDSsum += dphi[n + N*(m + M*l)][0]; //����CPU�ܶ����
				}
			}
		}
	...
	if (dmyid == 0) {
		...
		ofstream wafile("interenergy.txt", ios_base::out | ios_base::app);
		wafile << kd << " " << sum_enei / L / M / N << " " << sum_dens / L / M / N << endl;
	}
...
////////find the max/min value
   ...
   float pii;
   MPI_Allreduce(&phimax,&pii,1,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);//�����н��̽���һ��ȡ���ֵ��Լ�����Ϊpii
...
//   outfile << "      <PDataArray type=\"Float32\" Name=\"enei\"/>" << endl;
   ...
   sprintf(filenames, "%s%d%s%d%s%d%s", dfilename, theta1, "_",kd,"_",0,".vti");
   outfile << "    <Piece Extent=\"0 "<<(dlocal_n0/RD)-1<<" 0 "<<M/RD-1<<" "<<dstartz<< " "<<dendz-1<<"\" Source=\""<<filenames<<"\"/>"<<endl;
   for (int i=1; i<dnumprocs; i ++) {
   sprintf(filenames, "%s%d%s%d%s%d%s", dfilename, theta1,"_", kd,"_",i,".vti");
...
static void sVTKwriteBianry(fftw_complex *dphi,int dlocal_n0,int dlocal_0_start,int dmyid,int dnumprocs){
   char filename[20];
   sprintf(filename, "%s%d%s%d%s%d%s", "phi_",theta1,"_",kd,"_",dmyid,".vti");
...
static void sVTKwriteBianryc(fftw_complex *dcon,int dlocal_n0,int dlocal_0_start,int dmyid,int dnumprocs){
   char filename[20];
   sprintf(filename, "%s%d%s%d%s%d%s", "con_",theta1,"_",kd,"_",dmyid,".vti");
...
static void READPHIVTIN(fftw_complex *dphi, int dlocal_n0,int dmyid){
     ...
     FILE * inf_field;
     inf_field=fopen(filename.c_str(),"rb");
	 if(inf_field==NULL){
...
static void phimax_atmposi(fftw_complex *dphi,int dlocal_n0,int dlocal_0_start, int dmyid,int dnumprocs){
   ...
   float phic,phimax=0,phicut=0.40,bufphimax;
   for (int l=0; l <dlocal_n0; l ++) for (int m=0; m <M; m ++) for (int n=0; n <N; n ++){
           if(dphi[n+N*(m+M*l)][0]>phimax) {phimax=dphi[n+N*(m+M*l)][0];}
   }
...
//  cout<<"phic="<<phic<<" "<<"phimax="<<phimax<<" "<<"myid="<<dmyid<<endl;
 ...
 float *buf;
   ...
   for(int l=1; l <dlocal_n0+1; l ++) for (int m=0; m <M; m ++) for (int n=0; n <N; n ++){
       buf[n+N*(m+M*l)]=dphi[n+N*(m+M*(l-1))][0];
   }
   
    if(dmyid==head) {MPI_Sendrecv(&buf[M*N],M*N,MPI_FLOAT,tail,40,&buf[0],M*N,MPI_FLOAT,tail,40,MPI_COMM_WORLD,&stat); }
    if(dmyid==tail) {MPI_Sendrecv(&buf[(dlocal_n0)*M*N],M*N,MPI_FLOAT,head,40,&buf[(dlocal_n0+1)*M*N],M*N,MPI_FLOAT,head,40,MPI_COMM_WORLD,&stat);}

    MPI_Sendrecv(&buf[M*(dlocal_n0)*N],M*N,MPI_FLOAT,right,20,&buf[0],M*N,MPI_FLOAT,left,20,MPI_COMM_WORLD,&stat);
    MPI_Sendrecv(&buf[M*N],M*N,MPI_FLOAT,left,20,&buf[M*(dlocal_n0+1)*N],M*N,MPI_FLOAT,right,20,MPI_COMM_WORLD,&stat);        

    //*******************************************************************begin atom finding
float psum;
   double xtemp,ytemp,ztemp; int num_atoms=0;
   int lf,lb,mf,mb,nf,nb,lspace=1,mark,ix,jy,kz,nsum;
  // float pc[L*M][3];
    
   double *pc_x,*pc_y,*pc_z,*phi_max;  
   pc_x=(double*) malloc(M*dlocal_n0*N*sizeof(double));
  ...
  for (int l=1; l <dlocal_n0+1; l ++) for (int m=0; m <M; m ++) for (int n=0; n <N; n ++){
        psum=0.;nsum=0;mark=1;

 if(buf[n+N*(m+M*l)]>phic){lf=l-1;
                            lb=l+1;
                            mf=m-1;
                            mb=m+1;
                            nf=n-1;
        ...
        for(int lx=-1;lx<lspace+1;lx++) for(int ly=-lspace;ly<lspace+1;ly++) for (int lz=-lspace;lz<lspace+1;lz++){
          ix=l+lx;
          jy=m+ly; 
          kz=n+lz;
          ...
          if (buf[kz+N*(jy+M*ix)]>buf[n+N*(m+M*l)]) {mark=0;}  //if mark=1, corresponds to a maximum value
                ...
                if (mark==1){
                                  ...
                                  ztemp=n; 
                                          ...
                                          double z0=buf[n+N*(m+M*l)];
                                          double z1=buf[n+N*(m+M*lb)];
                                          double z3=buf[n+N*(m+M*lf)];
                                          double z4=buf[n+N*(mb+M*l)];
                                          double z2=buf[n+N*(mf+M*l)];
                                          double z5=buf[nb+N*(m+M*l)];
                                          double z6=buf[nf+N*(m+M*l)];
                                          double a0=z0;
                                          ...
                                          double a5=0.5*(z5-z6)/dx+10e-8;
                                          double a6=0.5*(z5+z6-2.*z0)/(dx*dx)+10e-8;

                                          double delta_x=-a1/(2.*a3);
                                          double delta_y=-a2/(2.*a4);
                                          double delta_z=-a5/(2.*a6);

                                      pc_x[num_atoms]=(xtemp+1)*dx+delta_x;
                                      pc_y[num_atoms]=(ytemp+1)*dx+delta_y;
                                      pc_z[num_atoms]=(ztemp+1)*dx+delta_z;
                                           phi_max[num_atoms]=a0-0.25*a1*a1/a3-0.25*a2*a2/a4-0.25*a5*a5/a6;
                                      num_atoms++;
                            }
                           
         
   }  // if(buf[n+N*(m+M*l)]>0.5)
}     //for
  ...
  // if (num_atoms==0) {pc_x[0]=-1; pc_y[0]=-1;pc_z[0]=-1; num_atoms=1;}// add false data if num_atoms is zero
   ...
   displs[0]=0;
   for(int i=1; i<dnumprocs; ++i){displs[i]=displs[i-1]+rcounts[i-1];}
   int size_atoms=displs[dnumprocs-1]+rcounts[dnumprocs-1];


    double *posi_x,*posi_y,*posi_z,*pmax;
      posi_x=(double*) malloc(size_atoms*sizeof(double));
     posi_y=(double*) malloc(size_atoms*sizeof(double));
     posi_z=(double*) malloc(size_atoms*sizeof(double));
     pmax=(double*) malloc(size_atoms*sizeof(double));

   MPI_Gatherv(pc_x,size_pc,MPI_DOUBLE,posi_x,rcounts,displs,MPI_DOUBLE,0,MPI_COMM_WORLD);   
   MPI_Gatherv(pc_y,size_pc,MPI_DOUBLE,posi_y,rcounts,displs,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Gatherv(pc_z,size_pc,MPI_DOUBLE,posi_z,rcounts,displs,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Gatherv(phi_max,size_pc,MPI_DOUBLE,pmax,rcounts,displs,MPI_DOUBLE,0,MPI_COMM_WORLD); 
   ...
   if (dmyid==0) {
    ...
    ofstream outfile0(filename,ios_base::out);
    outfile0.setf(ios_base::fixed,ios_base::floatfield);
    outfile0.precision(4);
    outfile0<<size_atoms<<endl;
    outfile0<<"Kommentar"<<endl;


        for (int i=0; i<size_atoms;i++) {
            if (posi_x[i]>=0) {outfile0 <<posi_x[i]<<" "<<posi_y[i]<<" "<<posi_z[i]<<" "<<pmax[i]<<endl;}
    }
	...
	for (int i = 0; i<dsize_buf; i++) {
		for (int j = 0; j<dsize_buf; j++) {
			if (i != j)
			{
				...
				r3[7] = r3[1] - frame_y;
				r3[8] = r3[2] - frame_z;
				drx = min3(abs(r3[0]), abs(r3[3]), abs(r3[6]));
				dry = min3(abs(r3[1]), abs(r3[4]), abs(r3[7]));
				drz = min3(abs(r3[2]), abs(r3[5]), abs(r3[8]));
				...
				if (rr<bench100_h) {
					...
					if (Nb[i] <= CNmax && Nb[i]>0) {
						rij[Nb[i] - 1 + CNmax*i] = rr;
						for (int ii = 0; ii<7; ii = ii + 3) {
							if (abs(fabs(r3[ii]) - drx)<1e-3) rjx = drbuf_x[i] - r3[ii];
						}	// the X position of neighbor j considering the periodic boundary condition	  
						for (int ii = 1; ii<8; ii = ii + 3) {
							if (abs(fabs(r3[ii]) - dry)<1e-3) rjy = drbuf_y[i] - r3[ii];
						}
						for (int ii = 2; ii<9; ii = ii + 3) {
							if (abs(fabs(r3[ii]) - drz)<1e-3) rjz = drbuf_z[i] - r3[ii];
						}
...
static void elastic(double *drbuf_x, double *drbuf_y, double *drbuf_z, int dsize_buf) {
	...
	double kels=1.0;
	int numzd_all=0;
	double zbxl=0,zbyl=0,zbzl=0,disof2a;
	double *strainx = 0, *strainy = 0, *num_near = 0,*stnener=0,*dis_near=0,*cos_near=0,*sin_near=0;
	int *num_crs=0;
	int m0=L/8,m1=M/8,m2=N/8;
	int *numi_near=0;
    double *posofzdx=0,*posofzdy=0,*stnof2a=0,*stnzdx_all=0,*stnzdy_all=0,*stnzd_all=0,*strain=0;
	double *strainx_near=0,*strainy_near=0;

	double tmpx, tmpy;
	
	strainx = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainy = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	num_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnener = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	num_crs = (int *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	dis_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	cos_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	sin_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	
	posofzdx= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	posofzdy= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnof2a= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnzdx_all= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnzdy_all= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnzd_all= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	
	strain= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainx_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainy_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	
	numi_near = (int *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
		...
		for (int j = 0; j<dsize_buf; j++) {
			if (i != j)
			{
				...
				if(drbuf_x[i]<=bndv&&abs(drbuf_x[i]-drbuf_x[j]+m0)<=bndv1){
				zbxl=drbuf_x[j]-m0;
			    if(drbuf_y[i]<=bndv&&abs(drbuf_y[i]-drbuf_y[j]+m1)<=bndv1){zbyl=drbuf_y[j]-m1;}
				if(drbuf_y[i]>=m1-bndv&&abs(drbuf_y[j]-drbuf_y[i]+m1)<=bndv1){zbyl=drbuf_y[j]+m1;}
             //   if(drbuf_z[i]<=bndv&&abs(drbuf_z[i]-drbuf_z[j]+m2)<=bndv1){zbzl=drbuf_z[j]-m2;}
			//	if(drbuf_z[i]>=m2-bndv&&abs(drbuf_z[j]-drbuf_z[i]+m2)<=bndv1){zbzl=drbuf_z[j]+m2;}
				
				}
				if(drbuf_x[i]>=m0-bndv&&abs(drbuf_x[j]-drbuf_x[i]+m0)<=bndv1){
				zbxl=drbuf_x[j]+m0;
			    if(drbuf_y[i]<=bndv&&abs(drbuf_y[i]-drbuf_y[j]+m1)<=bndv1){zbyl=drbuf_y[j]-m1;}
				if(drbuf_y[i]>=m1-bndv&&abs(drbuf_y[j]-drbuf_y[i]+m1)<=bndv1){zbyl=drbuf_y[j]+m1;}
              //  if(drbuf_z[i]<=bndv&&abs(drbuf_z[i]-drbuf_z[j]+m2)<=bndv1){zbzl=drbuf_z[j]-m2;}
			//	if(drbuf_z[i]>=m2-bndv&&abs(drbuf_z[j]-drbuf_z[i]+m2)<=bndv1){zbzl=drbuf_z[j]+m2;}				
				...
				if(drbuf_y[i]<=bndv&&abs(drbuf_y[i]-drbuf_y[j]+m0)<=bndv1){
				zbyl=drbuf_y[j]-m0;
			    if(drbuf_x[i]<=bndv&&abs(drbuf_x[i]-drbuf_x[j]+m1)<=bndv1){zbxl=drbuf_x[j]-m1;}
				if(drbuf_x[i]>=m1-bndv&&abs(drbuf_x[j]-drbuf_x[i]+m1)<=bndv1){zbxl=drbuf_x[j]+m1;}
             //   if(drbuf_z[i]<=bndv&&abs(drbuf_z[i]-drbuf_z[j]+m2)<=bndv1){zbzl=drbuf_z[j]-m2;}
			//	if(drbuf_z[i]>=m2-bndv&&abs(drbuf_z[j]-drbuf_z[i]+m2)<=bndv1){zbzl=drbuf_z[j]+m2;}				
				}
				if(drbuf_y[i]>=m0-bndv&&abs(drbuf_y[j]-drbuf_y[i]+m0)<=bndv1){
				zbyl=drbuf_y[j]+m0;
			    if(drbuf_x[i]<=bndv&&abs(drbuf_x[i]-drbuf_x[j]+m1)<=bndv1){zbxl=drbuf_x[j]-m1;}
				if(drbuf_x[i]>=m1-bndv&&abs(drbuf_x[j]-drbuf_x[i]+m1)<=bndv1){zbxl=drbuf_x[j]+m1;}
            //    if(drbuf_z[i]<=bndv&&abs(drbuf_z[i]-drbuf_z[j]+m2)<=bndv1){zbzl=drbuf_z[j]-m2;}
			...
			//	if(drbuf_z[i]<=bndv&&abs(drbuf_z[i]-drbuf_z[j]+m2)<=bndv1){
			//		zbzl=drbuf_z[j]-m2;
			//		if(drbuf_x[i]<=bndv&&abs(drbuf_x[i]-drbuf_x[j]+m1)<=bndv1){zbxl=drbuf_x[j]-m1;}
			//	    if(drbuf_x[i]>=m1-bndv&&abs(drbuf_x[j]-drbuf_x[i]+m1)<=bndv1){zbxl=drbuf_x[j]+m1;}
			//		if(drbuf_y[i]<=bndv&&abs(drbuf_y[i]-drbuf_y[j]+m1)<=bndv1){zbyl=drbuf_y[j]-m1;}
			//	    if(drbuf_y[i]>=m1-bndv&&abs(drbuf_y[j]-drbuf_y[i]+m1)<=bndv1){zbyl=drbuf_y[j]+m1;}
			//	}
			//	if(drbuf_z[i]>=m2-bndv&&abs(drbuf_z[j]-drbuf_z[i]+m2)<=bndv1){
			//		zbzl=drbuf_z[j]+m2;
			//		if(drbuf_x[i]<=bndv&&abs(drbuf_x[i]-drbuf_x[j]+m1)<=bndv1){zbxl=drbuf_x[j]-m1;}
			//	    if(drbuf_x[i]>=m1-bndv&&abs(drbuf_x[j]-drbuf_x[i]+m1)<=bndv1){zbxl=drbuf_x[j]+m1;}
			//		if(drbuf_y[i]<=bndv&&abs(drbuf_y[i]-drbuf_y[j]+m1)<=bndv1){zbyl=drbuf_y[j]-m1;}
			...
			//	}	
				...
				disof2a=sqrt(pow(zbxl-drbuf_x[i],2)+pow(zbyl-drbuf_y[i],2)+pow(zbzl-drbuf_z[i],2));
				
				//if(disof2a==0) cout<<"disof2a="<<" "<<disof2a<<endl;   //test
				
				if( disof2a>=0.8&&disof2a<=1.7 ){
					...
					if(midnum1>CNmax) {
					 ...
					 cos_near[i*CNmax+midnum1]=(zbxl-drbuf_x[i])/dis_near[i*CNmax+midnum1];  //����ԭ�����ڽ�ԭ�ӵĽǶȵ�cosֵ
					 sin_near[i*CNmax+midnum1]=(zbyl-drbuf_y[i])/dis_near[i*CNmax+midnum1];  //����ԭ�����ڽ�ԭ�ӵĽǶȵ�cosֵ
					 
					 if(disof2a>=0.9&&disof2a<=1.1&&abs(sin_near[i*CNmax+midnum1])<=0.1){
					 numzd_all=numzd_all+1;
					 num_crs[i]=num_crs[i]+1;
				     tmpx=(zbxl+drbuf_x[i])/2;
					 tmpy=(zbyl+drbuf_y[i])/2;
					 if(tmpx>m0){tmpx=tmpx-m0;}
                     if(tmpx<1){tmpx=tmpx+m0;}
                     if(tmpy>m0){tmpy=tmpy-m1;}
                     if(tmpy<1){tmpy=tmpy+m1;}  
					 
					 posofzdx[i*CNmax+num_crs[i]]=tmpx;
					 posofzdy[i*CNmax+num_crs[i]]=tmpy;
					 stnof2a[i*CNmax+num_crs[i]]=disof2a-8;
					 stnzdx_all[numzd_all]=tmpx;
					 stnzdy_all[numzd_all]=tmpy;
					 stnzd_all[numzd_all]=disof2a-8;
					 }
					 
					 if(dis_near[i*CNmax+midnum1]<10) {
					 strainx_near[i*CNmax+midnum1]=kels*(dis_near[i*CNmax+midnum1]-8)*cos_near[i*CNmax+midnum1];}
					 strainx[i]=strainx[i]+strainx_near[i*CNmax+midnum1];
					 if(dis_near[i*CNmax+midnum1]<10) {
					 strainy_near[i*CNmax+midnum1]=kels*(dis_near[i*CNmax+midnum1]-8)*sin_near[i*CNmax+midnum1];}
					 strainy[i]=strainy[i]+strainy_near[i*CNmax+midnum1];
					 stnener[i]=stnener[i]+pow(strainx_near[i*CNmax+midnum1],2)+pow(strainy_near[i*CNmax+midnum1],2);
			...
			strain[i]=sqrt(strainx[i]*strainx[i]+strainy[i]*strainy[i]); 	
			//moment[i]=crossk((zbx(numi)-zbx_zx),(zby(numi)-zby_zx),strainx[i],strainy[i]);	
	...
	char filename[20];
	sprintf(filename, "%s%d%s", "strain_", kd, ".txt");
	ofstream outfile(filename, ios_base::out);
	outfile.setf(ios_base::fixed, ios_base::floatfield);
	outfile.precision(4);
	...
	for (int i = 0; i<dsize_buf; i++) {
		if (drbuf_x[i] >= 0) {
			outfile << "st" << " " << drbuf_x[i] << " " << drbuf_y[i] << " " << drbuf_z[i] << " " << strain[i]<< endl;
		}
	}
...
static void elastic3D(double *drbuf_x, double *drbuf_y, double *drbuf_z, int dsize_buf) {
	...
	double *strainx_near=0,*strainy_near=0,*strainz_near=0;
	double *strain=0,*stnener=0,*strainx = 0,*strainy = 0,*strainz = 0;
	double *cos_near=0,*sin_near=0,*tha_near=0;
	...
	strainx = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainy = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
    strainz = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	//num_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	stnener = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	num_crs = (int *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strain= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainx_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	strainy_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
    strainz_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	...
	cos_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	sin_near= (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	tha_near = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	//fai = (double *)fftw_malloc(sizeof(double) * dsize_buf*CNmax);
	...
	for (int i = 0; i<dsize_buf; i++) {
			...
			if (i != j)
			{
				...
				if (rr<benchel&&rr>0.7) {
					...
					if (Nb[i] <= CNmax && Nb[i]>0) {
						...
						//cout << "rjz=" << rjz << endl;
						//nb_index[Nb[i] - 1 + CNmax*i] = j;   //the index of atom i's numbering (Nb[i]-1) neighbor
                        cos_near[Nb[i] - 1 + CNmax*i]=(rjx-drbuf_x[i])/rij[Nb[i] - 1 + CNmax*i];
						sin_near[Nb[i] - 1 + CNmax*i]=(rjy-drbuf_y[i])/rij[Nb[i] - 1 + CNmax*i];
						tha_near[Nb[i] - 1 + CNmax*i]=(rjz-drbuf_z[i])/rij[Nb[i] - 1 + CNmax*i];
						//cout<<"error1"<<endl;
				/*		if(rr>=0.9&&rr<=1.1&&abs(sin_near[Nb[i] - 1 + CNmax*i])<=0.1){
						numzd_all=numzd_all+1;
					    num_crs[i]=num_crs[i]+1;
				        tmpx=(rjx+drbuf_x[i])/2;
					    tmpy=(rjy+drbuf_y[i])/2;
						tmpz=(rjz+drbuf_y[i])/2;
						if(tmpx>m0){tmpx=tmpx-m0;}
                        if(tmpx<1){tmpx=tmpx+m0;}
                        if(tmpy>m1){tmpy=tmpy-m1;}
                        if(tmpy<1){tmpy=tmpy+m1;} 
                        if(tmpz>m2){tmpz=tmpz-m2;}
                        if(tmpz<1){tmpz=tmpz+m2;}  						
						
						
						
						}*/

                    if(rij[Nb[i] - 1 + CNmax*i]<1.3) strainx_near[Nb[i] - 1 + CNmax*i]=kels*(rij[Nb[i] - 1 + CNmax*i]-1)*cos_near[Nb[i] - 1 + CNmax*i];
                    strainx[i]=strainx[i]+strainx_near[Nb[i] - 1 + CNmax*i];
                    if(rij[Nb[i] - 1 + CNmax*i]<1.3) strainy_near[Nb[i] - 1 + CNmax*i]=kels*(rij[Nb[i] - 1 + CNmax*i]-1)*sin_near[Nb[i] - 1 + CNmax*i];
                    strainy[i]=strainy[i]+strainy_near[Nb[i] - 1 + CNmax*i];
					if(rij[Nb[i] - 1 + CNmax*i]<1.3) strainz_near[Nb[i] - 1 + CNmax*i]=kels*(rij[Nb[i] - 1 + CNmax*i]-1)*tha_near[Nb[i] - 1 + CNmax*i];
                    strainz[i]=strainz[i]+strainz_near[Nb[i] - 1 + CNmax*i];
					//cout<<"error2"<<endl;
					stnener[i]=stnener[i]+pow(strainx_near[Nb[i] - 1 + CNmax*i],2)+pow(strainy_near[Nb[i] - 1 + CNmax*i],2)+pow(strainz_near[Nb[i] - 1 + CNmax*i],2);
					//cout<<"error3"<<endl;
		...
		strain[i]=sqrt(strainx[i]*strainx[i]+strainy[i]*strainy[i]+strainz[i]*strainz[i]); 
	...
	for (int i = 0; i<dsize_buf; i++) {
		if (drbuf_x[i] >= 0) {
			outfile << "st" << " " << drbuf_x[i] << " " << drbuf_y[i] << " " << drbuf_z[i] << " " << strain[i]<<" "<<Nb[i]<< endl;
		}
	}
